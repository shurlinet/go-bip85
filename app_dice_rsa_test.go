...
// ========================================================================
// RSA: Basic generation + validation
// ========================================================================

func TestRSA_Generate2048(t *testing.T) {
	key, _ := ParseKey(specMasterXprv)
	defer key.Zero()
	...
	privKey, err := DeriveRSA(entropy, 2048)
	if err != nil {
		t.Fatal(err)
	}

	if privKey.N.BitLen() != 2048 {
		t.Errorf("key size: got %d bits, want 2048", privKey.N.BitLen())
	}

	// Validate the key.
	if err := privKey.Validate(); err != nil {
		t.Errorf("key validation: %v", err)
	}
}

func TestRSA_MultipleGenerations(t *testing.T) {
	// Go's crypto/rsa may mix in system randomness (FIPS compliance),
	// so we do NOT test for determinism. Instead we verify that each
	// call produces a valid, usable key.
	key, _ := ParseKey(specMasterXprv)
	defer key.Zero()

	for _, idx := range []uint32{0, 1, 2} {
		t.Run(fmt.Sprintf("index_%d", idx), func(t *testing.T) {
			path := RSAPath(2048, idx)
			entropy, err := DeriveEntropy(key, path)
			if err != nil {
				t.Fatal(err)
			}
...
func TestRSA_IndexIndependence(t *testing.T) {
	key, _ := ParseKey(specMasterXprv)
	defer key.Zero()

	e0, _ := DeriveEntropy(key, RSAPath(2048, 0))
	e1, _ := DeriveEntropy(key, RSAPath(2048, 1))
	defer ZeroBytes(e0)
	defer ZeroBytes(e1)

	k0, err := DeriveRSA(e0, 2048)
	if err != nil {
		t.Fatal(err)
	}
	k1, err := DeriveRSA(e1, 2048)
	if err != nil {
		t.Fatal(err)
	}

	if k0.N.Cmp(k1.N) == 0 {
		t.Error("different indexes produced identical RSA keys")
	}
}
...
func TestRSA_ValidKeySizes(t *testing.T) {
	key, _ := ParseKey(specMasterXprv)
	defer key.Zero()

	// Only test 1024 and 2048 to keep test time reasonable.
	// Larger sizes (4096, 8192) take significantly longer.
	for _, bits := range []int{1024, 2048} {
		t.Run(fmt.Sprintf("%d_bits", bits), func(t *testing.T) {
			path := RSAPath(uint32(bits), 0)
			entropy, err := DeriveEntropy(key, path)
			if err != nil {
				t.Fatal(err)
			}
			...
			privKey, err := DeriveRSA(entropy, bits)
			if err != nil {
				t.Fatal(err)
			}
			if privKey.N.BitLen() != bits {
				t.Errorf("key size: got %d bits, want %d", privKey.N.BitLen(), bits)
			}
		})
	}
}
...
// ========================================================================
// RSA: Error path tests
// ========================================================================

func TestRSA_InvalidKeyBits(t *testing.T) {
	entropy := make([]byte, 64)

	for _, bits := range []int{0, 512, 1023, 16385, 32768} {
		_, err := DeriveRSA(entropy, bits)
		if !errors.Is(err, ErrInvalidRSAKeyBits) {
			t.Errorf("bits=%d: got %v, want ErrInvalidRSAKeyBits", bits, err)
		}
	}
}

func TestRSA_ShortEntropy(t *testing.T) {
	_, err := DeriveRSA(make([]byte, 63), 2048)
	if !errors.Is(err, ErrInvalidLength) {
		t.Errorf("got %v, want ErrInvalidLength", err)
	}
}

func TestRSA_NilEntropy(t *testing.T) {
	_, err := DeriveRSA(nil, 2048)
	if !errors.Is(err, ErrInvalidLength) {
		t.Errorf("got %v, want ErrInvalidLength", err)
	}
}
...
// ========================================================================
// RSA: Entropy copy isolation
// ========================================================================

func TestEntropyCopyIsolation_RSA(t *testing.T) {
	entropy := make([]byte, 64)
	for i := range entropy {
		entropy[i] = byte(i)
	}
	...
	for i := range entropy {
		if entropy[i] != original[i] {
			t.Fatal("DeriveRSA modified the input entropy slice")
		}
	}
}

// ========================================================================
// RSA: GPGCreationTimestamp constant
// ========================================================================

func TestGPGCreationTimestamp(t *testing.T) {
	// Bitcoin genesis block timestamp: 2009-01-03 18:15:05 UTC.
	if GPGCreationTimestamp != 1231006505 {
		t.Errorf("GPGCreationTimestamp: got %d, want 1231006505", GPGCreationTimestamp)
	}
}

// ========================================================================
// RSA: Generated key usability (encrypt/decrypt round-trip)
// ========================================================================
...
func TestRSA_EncryptDecryptRoundTrip(t *testing.T) {
	...
	// Basic PKCS1v15 encrypt/decrypt round-trip using crypto/rand.
	plaintext := []byte("BIP85 RSA test message")
	...
	decrypted, err := rsa.DecryptPKCS1v15(nil, privKey, ciphertext)
	if err != nil {
		t.Fatalf("decrypt: %v", err)
	}

	if string(decrypted) != string(plaintext) {
		t.Error("RSA round-trip failed: decrypted text does not match plaintext")
	}
}
...
// ========================================================================
// DICE + RSA: bits.Len verification (anti-tamper for integer log2)
// ========================================================================
